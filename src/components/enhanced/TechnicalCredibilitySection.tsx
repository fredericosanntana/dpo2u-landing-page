...
const TechnicalCredibilitySection: React.FC = () => {
  ...
  const getStatusColor = (status: 'production' | 'beta' | 'planned') => {
    switch (status) {
      case 'production': return 'bg-brand-emerald-100 text-brand-emerald-700';
      case 'beta': return 'bg-yellow-100 text-yellow-700';
      case 'planned': return 'bg-brand-platinum-200 text-brand-platinum-800';
    }
  };
      ...
      <div className="container mx-auto container-padding">
        ...
        >
          <div className="inline-flex items-center px-4 py-2 bg-brand-sapphire-100 rounded-full mb-6">
            ...
            {liveMetrics && <div className="w-2 h-2 bg-brand-emerald-400 rounded-full ml-2 animate-pulse" />}
          </div>
              ...
              >
                <Card className="p-6 h-full hover:shadow-lg transition-shadow">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-semibold text-brand-chrome-800 mb-1">{metric.title}</h3>
                      <p className="text-sm text-brand-platinum-700">{metric.description}</p>
                    </div>
                    <Badge
                      className={`${metric.change >= 0 ? 'bg-brand-emerald-100 text-brand-emerald-700' : 'bg-brand-sapphire-100 text-brand-sapphire-700'}`}
                    >
                  ...
                  <div className="flex items-baseline space-x-2 mb-2">
                    ...
                    <span className="text-sm text-brand-platinum-600">{metric.unit}</span>
                  </div>

                  <div className="text-xs text-brand-platinum-600 mb-3">{metric.benchmark}</div>
                  ...
                  <Card key={item.name} className="p-6 hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right ml-6">
                        ...
                        <div className="text-xs text-brand-gray-500">Uptime</div>
                      </div>
                    </div>
                  </Card>
                ))}
